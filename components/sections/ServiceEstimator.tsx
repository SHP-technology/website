    ...
    <section className="py-20 bg-surface-subtle/40 border-y border-surface-border">
      <div className="container max-w-4xl">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold text-primaryText tracking-tight mb-4">
        ...
        <div className="glass-card p-8 md:p-10 rounded-3xl grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div className="flex flex-col gap-6">
            <div>
              <label className="text-xs font-bold text-primaryText block mb-2">
                1. Project Scope
              </label>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`py-2.5 px-3 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                      projectType === item.id
                        ? 'bg-brand-primary text-brand-buttonText border-brand-primary shadow-md'
                        : 'bg-surface-card border-surface-border text-secondaryText hover:border-brand-primary/50'
                    }`}
                  >
                    {item.label}
                  </button>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              />
              <label htmlFor="support-toggle" className="text-xs font-semibold text-secondaryText cursor-pointer">
                Include 6-month SLA Maintenance & Security Patches
              </label>
            </div>
          </div>

          <div className="p-8 rounded-2xl bg-surface-card border border-surface-border flex flex-col items-center justify-center text-center relative overflow-hidden">
            <Calculator className="w-8 h-8 text-brand-accent mb-2" />
            <span className="text-xs uppercase tracking-wider text-mutedText font-bold">Estimated Starting Price</span>
            <span className="text-4xl md:text-5xl font-extrabold text-brand-primary my-2">
              ${calculatedEstimate}
            </span>
            <span className="text-xs text-mutedText mb-6">Subject to final requirements review</span>
